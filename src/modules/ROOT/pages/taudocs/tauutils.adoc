...
-nccl::
    track NCCL events using the NCCL performance plugin API
-um::
...
-rocm_pc::
    captures the events provided by -rocm along with PC sampling from the ROCm api, tries to set stochastic, if not available, uses host trap
-rocm_pc_host::
    captures the events provided by -rocm along with PC sampling from the ROCm api, forces host trap sampling
-rocm_pc_stoc::
    captures the events provided by -rocm along with PC sampling from the ROCm api, forces stochastic sampling
...
-l0::
    captures events and metadata from the Level Zero performance API
-l0_pc::
    track pc sampling events through level zero API. Control sampling interval in microseconds with TAU_L0_SAMPLING_INTERVAL environmental variable.
...
Compares two TAU performance profiles or directories of profiles.

.Options
[horizontal]
`path1`::
Path to the first profile or directory. (first positional argument)
`path2`::
Path to the second profile or directory. (second positional argument)
`-h` `--help`::
show a help message and exit
`-o OUTPUT` `--output OUTPUT`::
Path for the output file or directory. Defaults to ./tau_diff_results
`-t THRESHOLD` `--threshold THRESHOLD`::
Report entries where any metric's % change exceeds this value. (Default: 0.0)
`--sort-by Calls|Subrs|Excl|Incl|alpha|original`::
Sort the report by the change in a metric column, alphabetically, or by the original order in the first profile. (Default: Incl)
`--check`::
Check-only mode. Exits with 0 if no differences are greater than the threshold, 1 otherwise. Suppresses all output files.
